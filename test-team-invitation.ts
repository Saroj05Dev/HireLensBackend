...
function extractCookies(cookieHeaders: string | string[]): string {
...
async function runTests(): Promise<void> {
  ...
  try {
    ...
    if (loginResponse.ok) {
      ...
      console.log("   Note: Make sure you have run 'npm run seed' first\n");
      return;
    }

    // Test 2: Create Invitation
    console.log("📝 Test 2: Create Invitation");
    const inviteResponse = await makeRequest(`${BASE_URL}/organization/invite`, {
      method: "POST",
      headers: {
        Cookie: adminCookies,
      },
    ...
    const pendingInvitesResponse = await makeRequest(`${BASE_URL}/organization/invites`, {
      method: "GET",
      headers: {
        Cookie: adminCookies,
      },
    ...
    const membersResponse = await makeRequest(`${BASE_URL}/organization/members`, {
      method: "GET",
      headers: {
        Cookie: adminCookies,
      },
    ...
    if (membersResponse.ok) {
      ...
      membersResponse.data.data.forEach((member: any, index: number) => {
    ...
    if (newUserId && organizationId) {
      ...
      const deactivateResponse = await makeRequest(
        `${BASE_URL}/organization/${organizationId}/members/${newUserId}/deactivate`,
        {
          method: "PATCH",
          headers: {
            Cookie: adminCookies,
    ...
    if (newUserId) {
      ...
      const verifyResponse = await makeRequest(`${BASE_URL}/organization/members`, {
        method: "GET",
        headers: {
          Cookie: adminCookies,
        },
      ...
      if (verifyResponse.ok) {
        const deactivatedMember = verifyResponse.data.data.find((m: any) => m.id === newUserId);
